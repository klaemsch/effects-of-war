...
events_is_vs_gz_by_domain <- events_is_vs_gz %>%
...
# events_is_vs_gz_by_domain list using above helper function
events_is_vs_gz_by_domain <- events_is_vs_gz_by_domain %>%
  rowwise() %>%
  mutate(ISOCountryCode = get_country_code_from_domain(source_domain))

# filter, so remaining is only not NA values
# summarize by country
events_is_vs_gz_by_country <- events_is_vs_gz_by_domain %>%
...
# write data to rds
saveRDS(events_is_vs_gz_by_country, file = "events/is_vs_gz/events_is_vs_gz_by_country.rds")
